    ...
    for (let i = 0; i < items_array.length; i++) {
        switch (items_array[i].rarity) {
            case 'C': {
                ...
                allSplittedItems.mutaforma.push({
                    id: items_array[i].id,
                    name: items_array[i].name,
                    rarity: items_array[i].rarity,
                    craftable: items_array[i].craftable
                ...
                allSplittedItems.inestimabili.push({
                    id: items_array[i].id,
                    name: items_array[i].name,
                    rarity: items_array[i].rarity,
                    craftable: items_array[i].craftable
                ...
                allSplittedItems.artefatti.push({
                    id: items_array[i].id,
                    name: items_array[i].name,
                    rarity: items_array[i].rarity,
                    craftable: items_array[i].craftable
                ...
                allSplittedItems.speciali.push({
                    id: items_array[i].id,
                    name: items_array[i].name,
                    rarity: items_array[i].rarity,
                    craftable: items_array[i].craftable
...
function addLootItem(itemName, currPrice, currDealer) {
    return new Promise(function (addLootItem_res) {
        return got.get("http://fenixweb.net:3300/api/v2/GbeUaWrGXKNYUcs910310/items/" + itemName.split(" ").join("_"), { responseType: 'json' }).then(function (full_infos) {
            ...
            if (typeof infos.res == 'undefined' || infos.res.length == 0) {
                ...
                return get_itemBaseFor(infos.res.id, parseInt(infos.res.craftable)).then(function (item_is_used_for) {
...
function addAllLootItems() {
    return new Promise(function (addLootItem_res) {
        return got.get("http://fenixweb.net:3300/api/v2/GbeUaWrGXKNYUcs910310/items/", { responseType: 'json' }).then(function (full_infos) {
            let infos = full_infos.body;
            if (typeof infos.res == 'undefined' || infos.res.length == 0) {
                addLootItem_res(false);
            } else {
                ...
                let insert_query = "INSERT INTO " + model.tables_names.items;
                insert_query += " (id, craftable, name, rarity, craft_pnt, estimate_value, base_value) VALUES ? ";
...
function get_itemBaseFor(item_id, is_craftable) {
    return new Promise(function (get_itemBaseFor_res) {
        if (is_craftable == 0) {
            ...
            return got.get("http://fenixweb.net:3300/api/v2/GbeUaWrGXKNYUcs910310/crafts/" + item_id + "/needed", { responseType: 'json' }).then(function (full_infos) {
                let infos = full_infos.body;
...
function updateItemMarketPrice(item_info, force) {
    return new Promise(function (updateItemMarketPrice_res) {
        ...
        if (condition) {
            return got.get("http://fenixweb.net:3300/api/v2/GbeUaWrGXKNYUcs910310/history/market_direct?limit=1000&fromItem=" + name, { responseType: 'json' }).then(function (full_infos) {
                let infos = full_infos.body;
...
async function getCraftList(toCraft_array, forArgonaut_id, check_zaino, preserve_zaino) {
    ...
    const zaino = await loadZainoOf(forArgonaut_id, check_zaino);
    let tmp_root_item;
...
function getItemFrom(itemID) {
    ...
    let item_fullInfo = allItemsArray.find(el => el.id == itemID); //prima da "module.exports.", ma credo sia uguale...
...
function getRarityCompleteName(rarity) {
    switch (rarity) {
        case "C": {
            return ("Comuni");
        } case "NC": {
...
module.exports.getRarityCompleteName = getRarityCompleteName;
