...
const banditw_file = '/controllers/models/bandit_w.txt';
...
// function setUserSuggestiosLimit(userId, newLimit) {
// 	return new Promise(function (setUserSuggestiosLimit_resolve) {
// 		sugg_pool.query("UPDATE " + tables_names.usr + " SET WARN = ? WHERE USER_ID  = ?"
// 			, [newLimit, userId],
// 			function (error, results) {
// 				if (!error) {
// 					setUserSuggestiosLimit_resolve(newLimit);
// 				}
// 				else {
// 					setUserSuggestiosLimit_resolve(-1);
// 				}
// 			});
// 	});
// }
// module.exports.setUserSuggestiosLimit = setUserSuggestiosLimit;
		...
		sugg_pool.query("UPDATE " + tables_names.usr + " SET USER_LAST_DISCUSSION = ? WHERE USER_ID = ?"
			...
			function (error, results) {
				if (!error) {
					...
					// 	SDATE: updated_time
					// };
					// return insertOn(tables_names.discussion, new_discussion).then(function (insert_res){
	...
	return new Promise(function (setSuggestionStatus_resolve) {
		if (check != null || check > 0) {
			...
			sugg_pool.query("UPDATE " + tables_names.sugg + " SET MSG_ID = ? WHERE SUGGESTION_ID LIKE ?",
				[msg_id, sugg_id], function (error, results) {
					if (!error) {
			...
			if (single_connection) {
				...
				Promise.all(votesCount).then(function (res) {
						...
						if (res[0] == null) {
							...
							let sugg_infos = {
								author: (res[0].author != null ? res[0].author : "NOAUTHOR"),
